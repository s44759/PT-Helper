...
//        InputStream inputStream = MtNotice.class.getClassLoader().getResourceAsStream("pt.properties");
        ...
        if (!"1".equals(ptProperties.getProperty("lmsignFlag"))) {
            return;
        }
        ...
        //读取要爬取的网址链接
        String uri = userProperties.getProperty("lm.uri");
        ...
        if (StringUtils.isBlank(uri)) {
            uri = "https://leaguehd.com/torrents.php";
        }

        //设置cookie,如果读取的数据为空,则打印提醒并退出程序
        String cookie = (String) userProperties.get("leaguehd.cookie");
        if (StringUtils.isBlank(cookie)) {
            System.out.println("柠檬新种发布监控,读取配置文件中cookie为空,请检查配置文件");
            return;
        }
        ...
        //创建刷新前的页面对象
        LmPage lmPageBef = new LmPage();

        //创建刷新后的页面对象
        LmPage lmPageAft = new LmPage();

        DateFormat simpleDateFormat = new SimpleDateFormat(FORMATPATTERN);
        System.out.println("[柠檬]" + "[" + simpleDateFormat.format(new Date()) + "]" + "开始初始化初始列表");

        //读取配置文件,使用配置信息通过工具类获取页面信息
        lmPageBef = LmUtils.getLmPage(uri, socketTimeout, connectTimeout, cookie);

        //如果获取的页面信息为空,打印并发送通知
        if (lmPageBef == null) {
            WindowInfo.showInfo("[柠檬]初始化失败", "查询得到的初始列表为空");
            System.out.println("[柠檬]" + "[" + simpleDateFormat.format(new Date()) + "]" + "初始化初始列表失败");
//            System.exit(0);
            return;
        } else {
            System.out.println("[柠檬]" + "[" + simpleDateFormat.format(new Date()) + "]" + "初始化初始列表完成");
        }
        ...
        while (true) {
            ...
            //使用配置信息通过工具类获取页面信息
            LmPage newPage = LmUtils.getLmPage(uri, socketTimeout, connectTimeout, cookie);

            //如果获取的页面信息为空,打印信息
            if (newPage == null) {
//                WindowInfo.showInfo("出现异常", "查询到的列表ptListAft为空");
                System.out.println("[柠檬]" + "[" + simpleDateFormat.format(new Date()) + "]" + "第" + num + "次查询\t" + "出现异常\t查询到的列表ptListAft为空");
            } else {
            ...
            if (findNum == 0) {
                System.out.println("[柠檬]" + "[" + simpleDateFormat.format(new Date()) + "]" + "第" + num + "次查询,无新种\t已查询到新种" + totalFindNum + "个");
            } else {
            ...
            //打印休眠信息并进入休眠
            System.out.println("[柠檬]" + "[" + simpleDateFormat.format(new Date()) + "]" + "休眠" + sleepTime + "秒");
            try {
